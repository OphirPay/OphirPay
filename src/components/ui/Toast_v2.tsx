...
/**
 * Accessibility-enhanced toast system with aria-live support.
 *
 * Changes from the original Toast.tsx (issue #204):
 * - Separate aria-live regions per urgency level (polite vs assertive).
 * - Payment-outcome toasts are announced with the correct urgency.
 * - Duplicate announcements are deduplicated across re-renders.
 */
...
const variantStyles: Record<ToastVariant, { icon: string; ring: string; live: "polite" | "assertive" }> = {
  success: {
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  // Track announced messages to deduplicate across re-renders
  ...
  const toast = useCallback(
    (t: Omit<ToastItem, "id">) => {
      ...
      // Deduplication: skip if the exact same announcement was recently fired
      const key = `${t.variant}|${t.title}|${t.description ?? ""}`;
      if (announcedRef.current.has(key)) {
        // Still add to visual stack but mark as already-announced so aria-live
        // regions remain silent for the duplicate.
        setToasts((prev) => [...prev.slice(-(MAX_VISIBLE - 1)), { ...t, id }]);
        return;
      }
      announcedRef.current.add(key);
      window.setTimeout(() => announcedRef.current.delete(key), 1000);
...
function ToastViewport({
  ...
  const politeToasts = toasts.filter((t) => variantStyles[t.variant].live === "polite");
  const assertiveToasts = toasts.filter((t) => variantStyles[t.variant].live === "assertive");

  return createPortal(
    <div className="fixed bottom-4 right-4 z-[60] flex flex-col gap-2 w-full max-w-sm pointer-events-none">
      {/* Assertive region for errors / warnings */}
      {assertiveToasts.length > 0 && (
        ...
        >
          {assertiveToasts.map((t) => (
            <p key={`announce-${t.id}`}>{t.title}{t.description ? `. ${t.description}` : ""}</p>
          ))}
        </div>
      )}
