...
//
// Webhook URL safety (SSRF guard) — the single implementation of the
// "is this a legitimate public delivery target?" decision.
//
// OphirPay's server makes outbound HTTP requests to URLs users register.
// Without a guard, a malicious user could register http://169.254.169.254/
// (cloud metadata), http://localhost:5432 (internal DB), or other internal
// endpoints and turn the server into a proxy.
//
// This guard blocks:
//   • Non-http(s) schemes
...
/** Outcome of a delivery-time URL validation. */
...
/** Boolean convenience wrapper around `validateWebhookUrlAtDelivery`. */
