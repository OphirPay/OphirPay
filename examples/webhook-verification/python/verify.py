...
`src/lib/webhook-deliver.ts`):
  ...
  4. Obtain the timestamp from the `X-OphirPay-Timestamp` header (or body).
  5. Compute HMAC-SHA256 (hex) over `{timestamp}.{canonical}` using your
     webhook secret.
  6. Compare against the `X-OphirPay-Signature` header with a
     constant-time comparison (`hmac.compare_digest`).
  7. Verify that the timestamp is within the tolerance window (default 300s).

CLI:

  python3 verify.py --secret <secret> --signature <hex> [--timestamp <iso>] \\
...
def verify_webhook_signature(body, signature, secret, timestamp=None, max_age_seconds=DEFAULT_MAX_AGE_SECONDS, now=None):
    ...
    to_sign = "%s.%s" % (effective_timestamp, canonical)
...
def main(argv=None):
    ...
    parser.add_argument("--timestamp", help="value of the X-OphirPay-Timestamp header")
    ...
    valid, reason = verify_webhook_signature(
        body, args.signature, args.secret, timestamp=args.timestamp, max_age_seconds=args.max_age, now=now
    )
    if valid:
        print("VALID")
        return 0
