 ...
 */
export const GET = withMetrics("GET /api/batches/summary", withRequestLogging(async function GET(
  request: Request
) {
  try {
    ...
    // Status counts — always expose every BatchStatus so the UI can render a
    // full card row even before a batch reaches a given state.
    const counts = Object.fromEntries(
      BATCH_STATUSES.map((s) => [s, 0])
    ...
    const perBatchStatus = new Map<string, Record<PaymentStatus, number>>();
    ...
    for (const p of paymentGroups) {
      ...
      const countsForBatch = perBatchStatus.get(p.batchId) ?? ({} as Record<PaymentStatus, number>);
    ...
    const drillDown = batches.map((batch) => {
      const pc = perBatchStatus.get(batch.id) ?? ({} as Record<PaymentStatus, number>);
      const completed = pc.COMPLETED ?? 0;
      const failed = (pc.FAILED ?? 0) + (pc.CANCELLED ?? 0);
      const batchTotal = Object.values(pc).reduce((sum, n) => sum + n, 0);
      return {
        ...batch,
        paymentCounts: {
          total: batchTotal,
