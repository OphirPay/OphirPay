...
//
// Webhook dispatch — the fan-out entry point used by API routes and server
// actions. Looks up active subscriptions, filters by event type, persists the
// event for replay and delivers to every matching endpoint in parallel.
//
// This module owns the *orchestration* (which webhooks, in what order) and
// delegates the three single responsibilities: filtering (`filter.ts`),
// delivery (`delivery.ts`) and persistence (`persistence.ts`).
...
import { recordWebhookDelivery, storeWebhookEvent } from "./persistence";
