...
describe('email', () => {
  it('requires configuration', async () => { vi.stubEnv('RESEND_API_KEY', ''); try { await expect(sendEmail({to:'t@t.com',subject:'S',html:'<p>H</p>'})).rejects.toThrow(/RESEND_API_KEY/); } finally { vi.unstubAllEnvs(); } });
