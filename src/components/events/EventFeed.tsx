...
// Lazy-loaded live event feed. Extracted from src/app/events/page.tsx so the
// WebSocket/SSE client and on-chain reader (and the @/lib/contracts module they
// pull in) is only fetched when the Events route is actually opened, keeping the
// initial bundle lean. Rendered through next/dynamic with a card skeleton
// fallback sized to the feed container so there is no layout shift.
