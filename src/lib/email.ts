...
/**
 * Perform a single request to the provider. Never throws: transport failures
 * are reported as `unreachable` so the caller can decide whether to retry.
 */
 ...
 * @throws {EmailSendError} when the provider rejected the message or could
 *         not be reached after the bounded retry budget.
 */
