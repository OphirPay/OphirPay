...
pub enum PaymentError {
    // Every variant below is reachable from at least one entrypoint.
    // Placeholder variants for features this contract does not implement
    // (staking, bridge, insurance, KYC, routing, gas, oracle, dispute
    // resolution, ...) were removed in #766 — see the reserved block at the
    // end of this enum and docs/SPEC.md § "Error code allocation".
    ...
    // ── Reserved codes — intentionally not declared as variants ─────────
    //
    // These values were declared by the original 307-variant catalog and
    // are kept free so a new error can be added later without renumbering
    // any code that is still reachable. Reserved ranges in 1..=307:
