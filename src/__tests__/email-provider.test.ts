...
describe("sendEmail — provider failure", () => {
  ...
  it("does not retry a permanent 4xx rejection", async () => {
    ...
    expect(sendError.message).toContain("API key is invalid");
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(vi.mocked(logger.error)).toHaveBeenCalledWith(
  ...
  it("retries transport failures and surfaces the last one", async () => {
    vi.stubEnv("RESEND_API_KEY", "re_test_key");
    const fetchMock = stubFetch(
    ...
    expect(error).toBeInstanceOf(EmailSendError);
    const sendError = error as EmailSendError;
    expect(sendError.message).toContain("network down");
    expect(sendError.attempts).toBe(3);
  ...
  it("recovers when a retry is accepted", async () => {
    vi.stubEnv("RESEND_API_KEY", "re_test_key");
    const fetchMock = stubFetch(
      jsonResponse({ message: "try again" }, 429),
      jsonResponse({ id: "email_ok" })
    );
...
describe("sendEmail — missing configuration", () => {
  it("throws EmailConfigurationError naming RESEND_API_KEY without calling the provider", async () => {
    vi.stubEnv("RESEND_API_KEY", "");
    const fetchMock = stubFetch(jsonResponse({ id: "never" }));

    const error = await captureRejection(payload);

    expect(error).toBeInstanceOf(EmailConfigurationError);
    expect((error as Error).message).toMatch(/RESEND_API_KEY/);
    expect((error as EmailConfigurationError).code).toBe("EMAIL_UNAVAILABLE");
  ...
  it("never turns a missing key into a fake success", async () => {
