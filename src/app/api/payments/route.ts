...
export const GET = withRequestLogging(async function GET(request: Request) {
  try {
    ...
    // Built via buildPaymentWhere, shared with GET /api/payments/export so the
    // list and its CSV export can never drift to different filter semantics.
    const baseWhere = buildPaymentWhere({ userId: auth.userId, status, search });
